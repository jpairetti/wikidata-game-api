...
Qué es este módulo:
    ...
    Los resultados se mapean a un formato común (id, nombre, genero,
    lanzamiento, plataforma, descripcion) y se guardan en store.CATALOGO_JUEGOS
...
Para qué sirve:
    ...
    - Poblar/ampliar el catálogo local con datos de Wikidata, usando
      `store.CATALOGO_JUEGOS` como estructura central (análogo a
      `store.USUARIOS`para los usuarios).

Qué hace:
    - buscar(q): busca en Wikidata, filtra videojuegos, mapea y guarda en
      catálogo; None si falla API.
    - obtener_juego(juego_id): devuelve juego desde catálogo o Wikidata;
      None si no existe o falla.
    - listar_o_buscar_juegos:
      GET /juegos (lista catálogo, búsqueda local o wikidata);
      502 si falla Wikidata.
...
def _get_user_agent() -> str:
    """
    Devuelve el User-Agent para las peticiones a Wikidata.
    """
    return os.environ.get("WIKIDATA_USER_AGENT",
                          "LabRedes2026/1.0").strip() or "LabRedes2026/1.0"


def _request(params: dict) -> dict | None:
    """
    Hace GET a la API de Wikidata con los params dados.
    Añade format=json y User-Agent.
    ...
    Returns:
        ...
        -None si hay error de red, timeout o JSON inválido.
    """
...
def _extraer_fecha(time_value: str) -> str:
    """
    Convierte un valor time de Wikidata a string YYYY-MM-DD.
...
def _claim_value_time(claims: dict, prop: str) -> str:
    """
    Extrae el valor time del primer claim de una propiedad
...
def _mapear_entidad(ent: dict, labels_cache: dict[str, str]) -> dict:
    """
    Convierte una entidad Wikidata a dict juego
    (id, nombre, genero, lanzamiento, plataforma, descripcion).
...
def _filtrar_ids_videojuegos(ids: list[str]) -> list[str] | None:
    ...
    # pide las entidades de la lista de ids (concatenadas de la forma
    # q1|q2|q3|...|qn)
    data = _request({
        "action": "wbgetentities",
        "ids": "|".join(ids),
...
def _entidades_completas_videojuegos(video_game_ids: list[str]) -> dict | None:
    """
    Obtiene entidades completas (labels, descriptions, claims) para los ids
    dados.
...
def _colectar_ref_qids(entities: dict) -> set:
    ...
    for ent in entities.values():
        ...
        for q in (
            _claim_value_id(
                claims, P_GENRE), _claim_value_id(
                claims, P_PLATFORM)):
            if q:
                ref_qids.add(q)
    return ref_qids
...
def _mapear_y_guardar_resultados(
    ...
    """
    Mapea entidades a formato juego, las guarda en CATALOGO_JUEGOS y devuelve
    la lista.
    ...
    Returns:
        Lista de dicts de juegos (id, nombre, genero, etc.);
        se omiten entidades missing.
    """
    resultados = []
...
def buscar(q: str) -> list[dict] | None:
    ...
    Returns:
        -Lista de juegos (dict con id, nombre, genero, etc.);
...
def obtener_juego(juego_id: str) -> dict | None:
    """
    Obtiene un juego por Q-id: primero catálogo local, si no está consulta
    Wikidata y guarda.
...
def _obtener_lista_juegos_para_get() -> tuple[list[dict] | None, tuple | None]:
    ...
    Con q y fuente=local: filtra por nombre en catálogo.
    Con q y fuente=wikidata: llama a buscar(q); si falla devuelve error 502.

    Returns:
        -(lista, None) con la lista de juegos a devolver;
        -(None, (response, 502)) si falla Wikidata.
    """
    ...
    if fuente == "wikidata":
        ...
        if resultados is None:
            return None, (jsonify(
                {"error": "Error al consultar Wikidata"}), 502)
        return list(resultados or []), None
    q_lower = q.lower()
    return [
        j for j in CATALOGO_JUEGOS.values() if q_lower in (
            j.get("nombre") or "").lower()], None


def listar_o_buscar_juegos():
    """
    Handler GET /juegos: listar catálogo o buscar por q con fuente local o
    wikidata.
    ...
    Returns:
        Response:
            -JSON con lista de juegos y 200;
            -502 si fuente=wikidata y falla la API.
    """
