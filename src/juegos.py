"""
Lista de juegos por usuario: agregar, listar, actualizar y eliminar ítems
...
Qué hace:
    - listar_juegos_usuario:    devuelve la lista enriquecida con datos del
                                catálogo, acepta genero, ordenar, orden.
    - agregar_juego_usuario:    agrega un juego por juego_id (debe existir en
                                catálogo o Wikidata), 409 si ya está.
    - actualizar_juego_usuario: actualiza los flags del ítem, 404 si el juego
                                no está en la lista.
...
from .wikidata import obtener_juego
from . import filtros
...
def _lista_usuario(usuario_id: int):
    ...
    Returns:
        -Lista de ítems, dict con:
            juego_id, tengo, quiero, jugado, me_gusta, fecha_agregado)
...
def _enriquecer_item(item: dict) -> dict | None:
    """
    Combina un ítem de la lista (juego_id + flags) con los datos del juego
    en el catálogo.
    ...
    Returns:
        -Dict con todos los campos del juego
            (id, nombre, genero, etc.) más los flags;
...
def listar_juegos_usuario(usuario_id: int):
    """
    Lista los juegos del usuario con filtro y ordenación; enriquecidos con
    datos del catálogo.
    ...
    Returns:
        -Response: JSON con lista de juegos (
            cada uno con id, nombre, flags, etc.) y 200;
...
def _validar_body_agregar_juego():
    ...
    Returns:
        -(error_response, None) si hay error (400);
        -(None, (juego_id, req)) si es válido.
    """
    if not request.json:
    ...
    for key in ("juego_id", "tengo", "quiero", "jugado", "me_gusta"):
        if key not in req:
            return (
                jsonify({"error": f"Datos inválidos: falta {key}"}), 400), None
    return None, (str(req["juego_id"]).strip(), req)


def _lista_y_existente(
        usuario_id: int,
        juego_id: str) -> tuple[list[dict] | None, dict | None]:
    """
    Obtiene la lista del usuario y el ítem existente
    (si el juego ya está en la lista).
...
def eliminar_juego_usuario(usuario_id: int, juego_id: str):
    ...
    Returns:
        Response:
            -JSON con mensaje de éxito y 200
            -404 si usuario o juego en lista no existe.
    """
    lista, item = _lista_y_existente(usuario_id, juego_id)
    if lista is None:
